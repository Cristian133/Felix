/*
 * Genera archivos con resultados.
 *
        ...
        char *FE_Usuario)
{
    ...
    if(((filecaespf = fopen(strcaespf, "w")) != NULL) && \
        ...
        if(!strcmp(CAE, STR_NULL)) {
            printf("CAE == STR_NULL\n");
            fprintf(filecaespf, STR_NULL);
            fprintf(filecaegen, STR_NULL);
            printf("Escribe %s vacio\n", strcaespf);
            printf("Escribe %s vacio\n", strcaegen);
            printf("Sin CAE\n");
        }
        else {
